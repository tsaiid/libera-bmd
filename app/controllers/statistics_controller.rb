class StatisticsController < ApplicationController
  ...
  def pcu_average
    ...
    # 計算各部位平均的 BMD
    ## Spine
    tmp_bmd = Spine.pcu.sum(:l1_bmd)
    @spine_l1_count = Spine.pcu.sum(:l1_included)
    @spine_avg_l1_bmd = tmp_bmd / @spine_l1_count
    @spine_l1_std = Math.sqrt(Spine.pcu.sum("(tot_bmd - #{@spine_avg_l1_bmd})*(tot_bmd - #{@spine_avg_l1_bmd})") / (@spine_l1_count - 1))

    tmp_bmd = Spine.pcu.sum(:l2_bmd)
    @spine_l2_count = Spine.pcu.sum(:l2_included)
    @spine_avg_l2_bmd = tmp_bmd / @spine_l2_count
    @spine_l2_std = Math.sqrt(Spine.pcu.sum("(tot_bmd - #{@spine_avg_l2_bmd})*(tot_bmd - #{@spine_avg_l2_bmd})") / (@spine_l2_count - 1))

    tmp_bmd = Spine.pcu.sum(:l3_bmd)
    @spine_l3_count = Spine.pcu.sum(:l3_included)
    @spine_avg_l3_bmd = tmp_bmd / @spine_l3_count
    @spine_l3_std = Math.sqrt(Spine.pcu.sum("(tot_bmd - #{@spine_avg_l3_bmd})*(tot_bmd - #{@spine_avg_l3_bmd})") / (@spine_l3_count - 1))

    tmp_bmd = Spine.pcu.sum(:l4_bmd)
    @spine_l4_count = Spine.pcu.sum(:l4_included)
    ...
    @hip_neck_std = Math.sqrt(Hip.pcu.sum("(neck_bmd - #{@hip_avg_neck_bmd})*(neck_bmd - #{@hip_avg_neck_bmd})") / (@hip_count - 1))

    tmp_bmd = Hip.pcu.sum(:troch_bmd)
    @hip_avg_troch_bmd = tmp_bmd / @hip_count
    @hip_troch_std = Math.sqrt(Hip.pcu.sum("(troch_bmd - #{@hip_avg_troch_bmd})*(troch_bmd - #{@hip_avg_troch_bmd})") / (@hip_count - 1))

    tmp_bmd = Hip.pcu.sum(:inter_bmd)
    @hip_avg_inter_bmd = tmp_bmd / @hip_count
    @hip_inter_std = Math.sqrt(Hip.pcu.sum("(inter_bmd - #{@hip_avg_inter_bmd})*(inter_bmd - #{@hip_avg_inter_bmd})") / (@hip_count - 1))

    tmp_bmd = Hip.pcu.sum(:htot_bmd)
    @hip_avg_htot_bmd = tmp_bmd / @hip_count
    @hip_htot_std = Math.sqrt(Hip.pcu.sum("(htot_bmd - #{@hip_avg_htot_bmd})*(htot_bmd - #{@hip_avg_htot_bmd})") / (@hip_count - 1))

    tmp_bmd = Hip.pcu.sum(:wards_bmd)
    ...
    @forearm_ru13_std = Math.sqrt(Forearm.pcu.sum("(ru13tot_bmd - #{@forearm_avg_ru13_bmd})*(ru13tot_bmd - #{@forearm_avg_ru13_bmd})") / (@forearm_count - 1))

    tmp_bmd = Forearm.pcu.sum(:rumidtot_bmd)
    @forearm_avg_rumid_bmd = tmp_bmd / @forearm_count
    @forearm_rumid_std = Math.sqrt(Forearm.pcu.sum("(rumidtot_bmd - #{@forearm_avg_rumid_bmd})*(rumidtot_bmd - #{@forearm_avg_rumid_bmd})") / (@forearm_count - 1))

    tmp_bmd = Forearm.pcu.sum(:ruudtot_bmd)
    @forearm_avg_ruud_bmd = tmp_bmd / @forearm_count
    @forearm_ruud_std = Math.sqrt(Forearm.pcu.sum("(ruudtot_bmd - #{@forearm_avg_ruud_bmd})*(ruudtot_bmd - #{@forearm_avg_ruud_bmd})") / (@forearm_count - 1))

    tmp_bmd = Forearm.pcu.sum(:rutot_bmd)
    @forearm_avg_rutot_bmd = tmp_bmd / @forearm_count
    @forearm_rutot_std = Math.sqrt(Forearm.pcu.sum("(rutot_bmd - #{@forearm_avg_rutot_bmd})*(rutot_bmd - #{@forearm_avg_rutot_bmd})") / (@forearm_count - 1))
